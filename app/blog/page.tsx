      ...
      <section className="mx-auto max-w-7xl px-4 pb-20 sm:px-6 lg:px-8">
        ...
        {posts.length === 0 ? (
          ...
          <>
            ...
            {/* Secondary featured — 2 larger cards */}
            {gridPosts.length > 0 && (
              <>
                  ...
                  {gridPosts.slice(0, 2).map((post, i) => (
                    <FadeUp key={post._id} delay={0.05 * i}>
                      <Link
                        ...
                        className="group card-glow flex h-full flex-col overflow-hidden rounded-2xl border border-border bg-surface transition hover:-translate-y-1"
                      >
                        <div className="relative aspect-[2/1] w-full overflow-hidden">
                          {post.featuredImage?.asset ? (
                            <Image
                              src={urlFor(post.featuredImage).width(700).height(350).quality(80).url()}
                          ...
                          {post.categories && post.categories.length > 0 && (
                            <div className="mb-3 flex flex-wrap gap-1.5">
                              {post.categories.map((cat) => (
                                <span key={cat.slug.current} className="rounded-full bg-accent/10 px-2.5 py-0.5 text-[11px] font-medium text-accent">
                                  {cat.title}
                                </span>
                              ))}
                            </div>
                          )}
                          <h3 className="text-xl font-bold leading-snug text-foreground line-clamp-2 group-hover:text-accent transition-colors">
                            {post.title}
                          </h3>
                          {post.excerpt && (
                            ...
                            {post.author && (
                              <span className="flex items-center gap-1.5">
                                {post.author.image?.asset ? (
                                  <Image src={urlFor(post.author.image).width(24).height(24).url()} alt={post.author.name} width={20} height={20} className="rounded-full object-cover" />
                                ) : (
                ...
                {/* Remaining posts — compact list style */}
                {gridPosts.length > 2 && (
                  <div className="mt-12">
                    ...
                    <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                      {gridPosts.slice(2).map((post, i) => (
                        <FadeUp key={post._id} delay={0.03 * i}>
                          <Link
                            href={`/blog/${post.slug.current}`}
                            className="group flex gap-4 rounded-xl border border-border bg-surface p-4 transition hover:border-accent/30 hover:bg-surface-2/50"
                          >
                            {/* Thumbnail */}
                            <div className="relative h-20 w-20 shrink-0 overflow-hidden rounded-lg">
                              ...
                              {post.isPremium && (
                                <span className="absolute right-1 top-1 rounded bg-accent px-1 py-px text-[8px] font-bold text-[#0f172a]">
                                  PRO
                                </span>
                              )}
                            </div>
                            ...
                            <div className="flex min-w-0 flex-1 flex-col justify-center">
                              {post.categories && post.categories.length > 0 && (
                                <span className="mb-1 text-[10px] font-medium uppercase tracking-wider text-accent">
                                  {post.categories[0].title}
                                </span>
                              )}
                              <h3 className="text-sm font-semibold leading-snug text-foreground line-clamp-2 group-hover:text-accent transition-colors">
                                {post.title}
                              </h3>
